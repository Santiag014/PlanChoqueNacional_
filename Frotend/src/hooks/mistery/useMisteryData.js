...
/**
 * Hook personalizado para manejar lo    try {
      ...
      const response = await fetch(`${API_URL}/api/mistery-shopper/guardar-hallazgo`, {
        method: 'POST',
        headers: {
...
export const useMysteryData = (userId) => {
  ...
  const cargarVisitas = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/mistery-shopper/pdvs`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
