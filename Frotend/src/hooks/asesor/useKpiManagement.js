...
export const useKpiManagement = (productSelection = {}) => {
  ...
  const [foto, setFoto] = useState([]); // Cambiar de null a array vacío para consistencia
  ...
  useEffect(() => {
    //console.log('🔄 DEBUG useKpiManagement: KPI cambió a:', kpiSeleccionado);
    ...
    // NO resetear fotos para KPI 'Implementación' para mantener las fotos de factura
    if (kpiSeleccionado !== 'Implementación') {
      //console.log('🗑️ DEBUG useKpiManagement: Reseteando fotos para KPI:', kpiSeleccionado);
      setFoto([]); // Cambiar de null a array vacío
    } else {
      //console.log('✅ DEBUG useKpiManagement: Manteniendo fotos para Implementación');
    }
  }, [kpiSeleccionado]);
