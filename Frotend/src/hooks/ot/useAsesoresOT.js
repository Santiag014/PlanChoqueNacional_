...
export function useAsesoresOT() {
  ...
  const fetchAsesores = async () => {
    ...
    try {
      ...
      // Debug: Verificar si los asesores tienen información de compañía
        // console.log('useAsesoresOT: Primeros 3 asesores con compañía:', 
        //   data.data.slice(0, 3).map(a => ({ id: a.id, name: a.name, compania: a.compania })));
      ...
      const cleanData = data.data.map(asesor => ({
        ...
        compania: asesor.compania || null, // Asegurar que compania esté disponible para filtros
