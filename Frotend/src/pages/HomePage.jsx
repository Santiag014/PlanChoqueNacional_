  ...
  useEffect(() => {
    ...
    const checkAndCleanSession = () => {
      if (isAuthenticated()) {
        ...
        } else if (userRoleNum === 6 || userRole === 'BACKOFFICE' || userRole === 'backoffice') {
          navigate('/backoffice/home', { replace: true });
        // } else if (userRoleNum === 7 || userRole === 'IMPLEMENTACION' || userRole === 'implementacion') {
        //   navigate('/implementacion/home', { replace: true }); // DESHABILITADO TEMPORALMENTE
        } else {
          localStorage.clear();
    ...
    try {
      ...
      if (result.success) {
        ...
        if (userRole === 'ASESOR' || userRole === 'asesor' || userRole === 1) {
          ...
          navigate('/organizacion-terpel/home', { replace: true });
        } else if (userRole === 'BACKOFFICE' || userRole === 'backoffice' || userRole === 6) {
          navigate('/backoffice/home', { replace: true });
        // } else if (userRole === 'IMPLEMENTACION' || userRole === 'implementacion' || userRole === 7) {
        //   navigate('/implementacion/home', { replace: true }); // DESHABILITADO TEMPORALMENTE
        } else {
      ...
      <footer>
        <div className="footer-content">
          ...
          <span className="footer-logo">
            <img src="/logoTerpel.png" alt="Terpel" className="logo-footer" />
          </span>
        </div>
      </footer>
    </div>
