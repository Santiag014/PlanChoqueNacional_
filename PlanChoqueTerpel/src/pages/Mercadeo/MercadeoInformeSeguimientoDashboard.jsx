...
import FiltrosAvanzados from '../../components/Mercadeo/Filtros/FiltrosAvanzados';
...
export default function MercadeoInformeSeguimientoDashboard() {
  ...
  // Asesores del sistema con sus PDVs asignados
  const asesoresData = [
    { 
      id: 1, 
      codigo: 'AS001', 
  ...
  const obtenerPdvs = () => {
    ...
    if (filtros.asesor) {
      const asesor = asesoresData.find(a => a.nombre === filtros.asesor);
      if (asesor) {
      ...
      // Sin filtro de asesor: obtener todos los PDVs
      pdvsResult = asesoresData.flatMap(asesor => asesor.pdvs);
    }
  ...
  const getMetricasData = () => {
    const pdvsMeta = asesorSeleccionado ? asesorSeleccionado.pdvs.length : pdvsActuales.length;
    ...
    return [
      {
        ...
        icon: IconCobertura,
        meta: pdvsMeta,
        implementado: asesorSeleccionado ? 
          datosBase.cobertura.filter(p => 
            asesorSeleccionado.pdvs.some(pdv => pdv.codigo === p.codigo) && 
            p.estado === 'Registrado'
          ).length :
          datosBase.cobertura.filter(p => p.estado === 'Registrado').length,
        porcentaje: Math.round((asesorSeleccionado ? 
          datosBase.cobertura.filter(p => 
            asesorSeleccionado.pdvs.some(pdv => pdv.codigo === p.codigo) && 
            p.estado === 'Registrado'
          ).length :
          datosBase.cobertura.filter(p => p.estado === 'Registrado').length) / pdvsMeta * 100),
        color: '#e30613',
      ...
      },
      {
        ...
        titulo: 'Volumen',
        icon: IconVolumen,
        meta: asesorSeleccionado ? 
          asesorSeleccionado.pdvs.reduce((total, pdv) => total + pdv.meta_volumen, 0) :
          pdvsActuales.reduce((total, pdv) => total + pdv.meta_volumen, 0),
        implementado: asesorSeleccionado ? 
          datosBase.volumen.filter(p => 
            asesorSeleccionado.pdvs.some(pdv => pdv.codigo === p.codigo)
          ).reduce((total, pdv) => total + pdv.real, 0) :
          datosBase.volumen.reduce((total, pdv) => total + pdv.real, 0),
        porcentaje: asesorSeleccionado ? 
          Math.round((datosBase.volumen.filter(p => 
            asesorSeleccionado.pdvs.some(pdv => pdv.codigo === p.codigo)
          ).reduce((total, pdv) => total + pdv.real, 0) / 
          asesorSeleccionado.pdvs.reduce((total, pdv) => total + pdv.meta_volumen, 0)) * 100) :
          Math.round((datosBase.volumen.reduce((total, pdv) => total + pdv.real, 0) / 
          pdvsActuales.reduce((total, pdv) => total + pdv.meta_volumen, 0)) * 100),
        color: '#ff6b35',
      ...
      },
      {
        ...
        meta: pdvsMeta * 20,
        implementado: asesorSeleccionado ? 
          datosBase.frecuencia.filter(p => 
            asesorSeleccionado.pdvs.some(pdv => pdv.codigo === p.codigo)
          ).reduce((total, p) => total + p.visitas, 0) :
          datosBase.frecuencia.reduce((total, p) => total + p.visitas, 0),
        porcentaje: Math.round((asesorSeleccionado ? 
          datosBase.frecuencia.filter(p => 
            asesorSeleccionado.pdvs.some(pdv => pdv.codigo === p.codigo)
          ).reduce((total, p) => total + p.visitas, 0) :
          datosBase.frecuencia.reduce((total, p) => total + p.visitas, 0)) / (pdvsMeta * 20) * 100),
        color: '#f7931e',
      ...
      },
      {
        ...
        meta: pdvsMeta,
        implementado: asesorSeleccionado ? 
          datosBase.profundidad.filter(p => 
            asesorSeleccionado.pdvs.some(pdv => pdv.codigo === p.codigo) && 
            p.estado === 'Registrado'
          ).length :
        ...
        porcentaje: Math.round((asesorSeleccionado ? 
          ...
          datosBase.profundidad.filter(p => p.estado === 'Registrado').length) / pdvsMeta * 100),
        color: '#00a651',
      ...
      },
      {
        ...
        meta: pdvsMeta,
        implementado: asesorSeleccionado ? 
          datosBase.precios.filter(p => 
            asesorSeleccionado.pdvs.some(pdv => pdv.codigo === p.codigo) && 
            p.estado === 'Precios Reportados'
          ).length :
        ...
        porcentaje: Math.round((asesorSeleccionado ? 
          ...
          datosBase.precios.filter(p => p.estado === 'Precios Reportados').length) / pdvsMeta * 100),
        color: '#0066cc',
